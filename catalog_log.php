	...
	<body>
		...
		<?php
			...
			{	
				...
				$sql=("SELECT DISTINCT * FROM `institute_details` WHERE 1");
				$result=$db->query($sql);
				echo "<div class=\"col-sm-6\"><table class=\"table table-bordered table-hover\">
						<th>Institute Name</th>
						<th>Total Catalogs</th>
						";
				foreach($result as $r)
					{
						$name=$r['name'];
						$sql2=("SELECT DISTINCT `title` FROM `catalog` where `institute`='$name'");
						$result2=$db->query($sql2);
						$row=mysqli_fetch_array($result2);
						echo "<tr>
								<td>$name</td>
								<td>$result2->num_rows</td>
								</tr>
								";
		...
		<?php
			$sql=("SELECT DISTINCT `title` FROM `catalog`");
			$result=$db->query($sql);
			echo "<th>Commulative Catalog Count</th><tr>
