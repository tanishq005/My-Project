...
<html>
	...
	<body>
		...
		$name=$row['name'];
		$sql4holdings=("SELECT `holdings` FROM `catalog` WHERE `title`='$title' AND `institute`='$name' ");
		$result4holdings=$db->query($sql4holdings);
		$r=mysqli_fetch_assoc($result4holdings);
		$holdings=$r['holdings'];

		
		echo "<div class=\"panel panel-default\">
				<div class=\"panel-body\">
					Edit Holdings for <b>$title</b>
				</div>
			  </div>
			 "; 
		...
		echo " <div class=\"col-sm-12\">
					<form action=\"process_edit_holdings.php\" method=\"post\"> 
						<div class=\"form-group\">
							<label>Title</label>
							<input type=\"text\" class=\"form-control\" name=\"title\"  value=\"$title\" readonly/>
							<label>Holdings</label>
							<textarea name=\"holdings\" class=\"form-control\" />$holdings</textarea>
